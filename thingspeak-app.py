...
if st.button("Fetch Air Quality Data🚀"):
    ...
    if data:
        ...
        if "Humidity" in fields:
            fig.add_trace(go.Scatter(x=time_stamps, y=humidity_values, mode='lines+markers', name='Humidity', line=dict(color='cyan')))
        if "Temperature" in fields:
            fig.add_trace(go.Scatter(x=time_stamps, y=temp_values, mode='lines+markers', name='Temperature', line=dict(color='magenta')))
        ...
        if "Humidity" in fields:
            if humidity_values:  # Check if there are valid humidity values
                ax1.plot(humidity_values, label="Humidity", color='c')

        if "Temperature" in fields:
            if temp_values:  # Check if there are valid temperature values
